...
$0.2\%$ of the global optimum after a single Bayesian batch (12 evaluations)
in every repetition, whereas a flat prior is still $18\%$ short of the
...
(positive semi-definite) covariance. In practice $\rho$ is obtained from an
...
restores numerical safety. Second, since $\rho(x,x) = 1$ exactly by
\eqref{eq:corr}, the prior variance at any point is $c^2 \sigma(x)^2$ by
construction --- there is no additional global outputscale to interfere with
the calibration.
