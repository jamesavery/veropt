...
def _read_setup_args():
    with open('setup_args.txt', 'r') as f:
...
class GlobalFlexibleResolutionSetup(VerosSetup):
    ...
    def set_parameter(self, state):
        ...
        setup_args = _read_setup_args()

        settings = state.settings

        c_k = setup_args['c_k']
        print(type(c_k), c_k)
        c_eps = setup_args['c_eps']
        ...
        # settings.c_k = 0.1  # default
        settings.c_k = setup_args['c_k']
        print(f"settings.c_k = {settings.c_k}")

        # settings.c_eps = 0.7  # default
        settings.c_eps = setup_args['c_eps']
        print(f"settings.c_eps = {settings.c_eps}")

        settings.alpha_tke = 30.0  # default
        # settings.alpha_tke = setup_args['alpha_tke']
        # print(f"settings.alpha_tke = {settings.alpha_tke}")
        
        settings.mxl_min = 1e-8
        # settings.mxl_min = setup_args['mxl_min']
        # print(f"settings.mxl_min = {settings.mxl_min}")

        settings.tke_mxl_choice = 2

        settings.kappaM_min = 2e-4
        settings.kappaH_min = 2e-5
        # settings.kappaM_min = setup_args["kappaM_min"]
        # print(f"settings.kappaM_min = {settings.kappaM_min}")
        ...
        settings.eke_c_k = setup_args['eke_c_k']
        print(f"settings.eke_c_k = {settings.eke_c_k}")

        # settings.eke_c_eps = 0.5

        settings.eke_c_eps = setup_args['eke_c_eps']
        print(f"settings.eke_c_eps = {settings.eke_c_eps}")
    ...
    def set_diagnostics(self, state):
        ...
        diagnostics["snapshot"].output_frequency = 360 * 86400.0
    ...
    @veros_routine(dist_safe=False, local_variables=["zt", "prho", "mld"])  # mld
    def after_timestep(self, state):
        vs = state.variables

        reference_depth = -10.
        reference_index = npx.max(npx.where(vs.zt < reference_depth)[0])
        prho_reference = vs.prho[:,:,reference_index] + 0.03

        prho_below_reference = vs.prho[:,:,:reference_index+1]
        zt_below_reference = vs.zt[:reference_index+1]
        zt_mask = npx.where(npx.isnan(prho_below_reference), npx.nan, zt_below_reference[npx.newaxis,npx.newaxis,:])
...
def set_forcing_kernel(state):
    ...
    c_taux = setup_args['c_taux']
    print(type(c_taux), c_taux)
    c_tauy = setup_args['c_tauy']
    print(type(c_tauy), c_tauy)
    ...
    vs.surface_taux = c_taux * vs.surface_taux
    print(f"c_taux = {c_taux}")
    vs.surface_tauy = update(vs.surface_tauy, at[:, :-1], f1 * vs.tauy[:, 1:, n1] + f2 * vs.tauy[:, 1:, n2])
    vs.surface_tauy = c_tauy * vs.surface_tauy
    print(f"c_tauy = {c_tauy}")
